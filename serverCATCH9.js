...
- "Captura total del buque (Kg)" -> net_catch_kg (den samlede fangst for fartoejet). Dette er det ENESTE felt der skal udfyldes naar data kommer fra Annex B.
- est_vaegt_kg -> saet ALTID null naar data kommer fra Annex B
- verified_kg -> saet ALTID null naar data kommer fra Annex B

VIGTIGT: Naar vaegterne hentes fra Annex B, skal KUN net_catch_kg udfyldes (med "Captura total del buque"). Felterne est_vaegt_kg og verified_kg skal forblive null - de maa IKKE udfyldes med "Captura transformada a base de la captura total", selvom denne kolonne findes i Annex B. "Captura transformada" er IKKE det samme som "Verified weight landed" og maa ikke bruges i boks 3.

Hvis CC boks 3 siger "Ver anexo B" skal du laese vaegterne fra Annex B i stedet, med ovenstaaende regel.
Saet null hvis et felt ikke fremgaar af nogen af disse kilder.
ADVARSEL: Brug ALDRIG tal fra Processing Statement (catch processed / processed fishery product) til disse felter, og brug ALDRIG "Captura transformada" fra Annex B til verified_kg eller est_vaegt_kg.
...
VIGTIGT OM TRANSPORTOPLYSNINGER (BOKS 10) - PLOMBENUMMER:
CC i CATCH-systemet maa UDELUKKENDE udfyldes med data der staar paa selve CC-dokumentets transport-appendix (sidste side, "INFORMACION RELATIVA AL TRANSPORTE / TRANSPORT DETAILS"). transport_plombe og transport_container skal hentes fra dette appendix - ALDRIG fra Bill of Lading, Health Certificate eller andre dokumenter, selvom de indeholder plombenumre.
Hvis CC'ets transport-appendix IKKE naevner plombenummer, skal transport_plombe saettes til null (feltet skal stå tomt). Brug KUN "N/A" som tekst hvis det er oplyst at CATCH-systemet kraever en vaerdi i feltet - som default skal det vaere null/tomt, ikke "N/A".
transport_container kan godt hentes fra CC's transport-appendix ("Numero(s) de los contenedores"), men hvis kun containernummer findes der (og ikke plombenummer), saet transport_plombe til null.
...
Returner dette JSON-objekt:
{
  ...
  "transport_skib": "skibsnavn og flag fx MSC NIGERIA Liberia",
  "transport_skib_imo": "skibets IMO-nummer hvis tilgaengeligt eller null",
  "transport_voyage_nr": "voyage number fra Bill of Lading. Findes under VESSEL AND VOYAGE NO. paa B/L - det er den del der kommer EFTER skibsnavnet og en bindestreg. Eksempel: hvis der staar MSC NIGERIA - NN620R er voyage number NN620R. Findes ogsaa i CATCH certifikatets transport appendix. Returner kun voyage-delen uden skibsnavn",
  "transport_doc_reference": "transport document reference - Bill of Lading nummer fx MEDUF9698293 eller null",
  "transport_container": "containernummer",
  "transport_plombe": "plombenummer fra CC's eget transport-appendix, hvis flere adskil med komma. Saet null (tomt) hvis CC's transport-appendix ikke naevner plombenummer - hent IKKE fra B/L eller andre dokumenter. Brug kun N/A hvis systemet kraever en udfyldt vaerdi",
  ...
  "ps_dok_efter_10jan2026": "true hvis PS er udstedt efter 10 januar 2026 ellers false - afgores af ps_dato",
  "ps_sundhedscert_nr": "sundhedscertifikat nummer",
  "ps_sundhedscert_dato": "sundhedscertifikat dato",
  "ps_endorsing_authority_navn": "navn paa den myndighed der har godkendt PS fx Subsecretaria de Recursos Pesqueros",
  "ps_endorsing_authority_land": "land for endorsing authority",
  "ps_endorsing_person_navn": "fulde navn paa den myndighedsperson der har underskrevet PS",
  "ps_endorsing_person_dato": "dato for myndighedspersonens underskrift",
  "ps_endorsing_signature_seal": "Ja hvis baade signatur og stempel er til stede",
  "ps_ansvarlig_signatur": "Ja hvis ansvarlig persons signatur er til stede",

VIGTIGT OM IM-VAEGTFELTER (BOKS 11):
- im_nettovægt_kg ("Nettovaegt i kg") skal udfyldes med vaegten af det faerdige produkt inklusive lage (fisk inkl. lage). Denne skal matche sundhedscertifikatet.
- im_net_fishery_product_weight_kg ("Nettovaegt af fiskevarer i kg") skal udfyldes med vaegten af ren fisk. Denne vaerdi er angivet paa Processing Statement (PS) - feltet "Producto de la pesca transformado (kg)" / "Processed fishery product (kg)", summeret for de fartoejer/linjer der hoerer til sendingen.
Hvis der ikke findes en Processing Statement i dokumenterne, saet im_net_fishery_product_weight_kg til samme vaerdi som im_nettovægt_kg (default naar ingen PS-kilde findes).
  ...
  "im_net_fishery_product_weight_kg": "net fishery product weight i kg som tal - vaegt af REN FISK fra Processing Statement (Processed fishery product kg), IKKE bruttovaegt. Hvis ingen PS findes, brug samme vaerdi som im_nettovægt_kg",
  ...
  "im_eksportland": "eksportland fx Ecuador",
  "im_port_afgang": "port airport or other place of departure fx Guayaquil",
  "im_bestemmelsessted": "point of destination fx Aarhus eller Danmark",
  "im_skib": "skibets navn og flag state",
  "im_voyage_nr": "voyage number",
  "im_transport_doc_reference": "transport document reference B/L nummer",
...
// Forsoeger at parse Claude's JSON-svar. Hvis svaret er afskaaret midt i en
// streng eller objekt (fordi max_tokens blev naaet), reparerer vi det saa
// vidt muligt i stedet for at smide hele udtraekket vaek.
function parseClaudeJson(rawText) {
  ...
  try {
    ...
    console.warn('Foerste JSON.parse fejlede, forsoeger reparation:', firstError.message);
  }

  let repaired = repairTruncatedJson(clean);
  try {
    const result = JSON.parse(repaired);
    console.warn('JSON blev reparereret efter afskaaret svar - tjek om alle fartoejer/linjer kom med.');
    result._catch_advarsel_afskaaret_svar = true;
    return result;
  } catch (secondError) {
    throw new Error('Svar kunne ikke parses, heller ikke efter reparation: ' + secondError.message);
  }
}

// Lukker en JSON-streng/struktur der er blevet afskaaret midt i, ved at
// fjerne det sidste, ufuldstaendige element og lukke alle aabne brackets.
function repairTruncatedJson(text) {
  let s = text;

  // Hvis den slutter midt i en streng (ulige antal u-escapede citationstegn
  // efter sidste komplette token), skaer vi tilbage til sidste gyldige
  // komma eller aabning og lukker strukturen der.
  let inString = false;
  let escape = false;
  let lastSafeIndex = -1;
  const stack = [];
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    else if (ch === ',' && stack.length > 0) { lastSafeIndex = i; }
  }

  if (inString || lastSafeIndex === -1) {
    // Svaret endte midt i en streng eller vi fandt intet sikkert punkt -
    // skaer tilbage til sidste komma uden for strenge.
    if (lastSafeIndex > -1) {
      s = s.substring(0, lastSafeIndex);
    }
  }

  // Genberegn hvilke brackets der stadig er aabne efter beskaering, og luk dem.
...
app.post('/api/catch-extract', async (req, res) => {
  ...
  try {
    ...
    const rawText = await callClaude(apiKey, b64, CATCH_PROMPT, 8000);
    console.log('Claude svar laengde:', rawText.length, 'tegn');
    console.log('Claude svar (start):', rawText.substring(0, 200));
    ...
    try {
      ...
      console.error('Sidste 300 tegn af svaret:', rawText.substring(Math.max(0, rawText.length - 300)));
